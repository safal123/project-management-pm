...
class TaskController extends Controller
{
    ...
    public function reorder(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $validated) {
            ...
            // Batch update all task orders in a single query using PostgreSQL VALUES join
            if (! empty($taskIds)) {
                $bindings = [];
                $placeholders = [];
                ...
                DB::statement(
                    "UPDATE tasks SET \"order\" = v.new_order::integer FROM (VALUES {$values}) AS v(id, new_order) WHERE tasks.id::text = v.id",
                    $bindings
                );
            }
        });
